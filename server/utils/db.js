...
class Db {
  ...
  async clear() {
    return await this.pool.query(
      "TRUNCATE TABLE days, words, words_to_rooms RESTART IDENTITY"
    );
  }
  ...
  async writeFoundWord(word, name, roomId) {
    ...
    sql =
      "INSERT INTO words_to_rooms (word_id, room_id, player_name) VALUES ($1, $2, $3) RETURNING id";
    result = await this.pool.query(sql, [wordId, roomId, name]);
    return result.rows[0].id;
  }

  async readDay() {
    let sql = "SELECT * FROM days LIMIT 1";
    let result = await this.pool.query(sql);
  ...
  async readFoundWords(roomId) {
    let sql =
      "SELECT word, player_name as name FROM words_to_rooms JOIN words ON word_id=words.id WHERE room_id=$1";
